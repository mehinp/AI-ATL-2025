...
export default function LiveGameCard({ game, onClick, onTeamSelect }: LiveGameCardProps) {
  const handleTeamSelect = (teamName: string, event: MouseEvent) => {
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <button
            ...
            onClick={(event) => handleTeamSelect(game.awayTeam.name, event)}
          >
          ...
          <button
            type="button"
            className="flex w-full items-center justify-between text-left"
            onClick={(event) => handleTeamSelect(game.homeTeam.name, event)}
          >
            <div className="flex items-center gap-3">
              <TeamLogo
        ...
        {game.hasFlashPick && (
          <Badge
            className="w-full justify-center bg-primary/10 text-primary hover:bg-primary/20 animate-pulse"
            data-testid="badge-flash-pick"
          >
